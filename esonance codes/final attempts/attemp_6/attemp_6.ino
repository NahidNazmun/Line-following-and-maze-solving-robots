...
int min_rc[8] = {201, 115, 129, 201, 130, 87, 101, 187};
int max_rc[8] = {1346, 756, 809, 1195, 811, 576, 673, 1278};
...
start:
  ...
  {
    int another_sensor = 0; // how many time to wait for checking other sensor
    ...
    do {
      bluetooth();
      analogread();
      while (another_sensor < a)
      {
        if (right_sensor > a_right_high)
        {
tsection:
          unsigned long tm = millis(); //line change
          unsigned long tm1;
          do {
            bluetooth();
            analogread();
  ...
  {
    another_sensor = 0; // how many time to wait for checking other sensor
    digitalWrite(l_motor_f, HIGH);
    digitalWrite(l_motor_b, LOW);
    digitalWrite(r_motor_f, HIGH);
    digitalWrite(r_motor_b, LOW);
