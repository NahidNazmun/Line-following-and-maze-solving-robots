...
int min_rc[8] = {200, 115, 129, 187, 144, 101, 115, 215};
int max_rc[8] = {1414, 863, 919, 1194, 961, 712, 809, 1525};
...
#define black_turn_speed 100
#define white_turn_speed 150
int turn_speed;
...
start:
  ...
  if (right_sensor > a_right_high && (sensor_values[3] > high || sensor_values[4] > high))
  {
...
tsection:
  ...
  else if (left_sensor > a_left_high && (sensor_values[3] > high || sensor_values[4] > high))
  {
    digitalWrite(l_motor_f, HIGH);
    digitalWrite(l_motor_b, LOW);
    digitalWrite(r_motor_f, HIGH);
  ...
  else if ( sensor_values[2] < low && sensor_values[3] < low && sensor_values[4] < low && sensor_values[5] < low && position > 2000 && position < 5000)
  {
    Serial1.println("Blank start");
    do{
    digitalWrite(l_motor_f, HIGH);
    digitalWrite(l_motor_b, LOW);
    digitalWrite(r_motor_f, HIGH);
    ...
    analogWrite(r_pwm, max_speed);
    position= readline();
    }while(sensor_values[2] < high && sensor_values[3] < high && sensor_values[4] < high && sensor_values[5] < high);
    Serial1.println("Blank end");
  }
