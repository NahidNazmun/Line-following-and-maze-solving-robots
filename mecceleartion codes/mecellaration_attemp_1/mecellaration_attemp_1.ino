...
void loop()
{
  ...
  position = readline();
  //t_section
  if (sensorValues[2] > high && sensorValues[3] > high && sensorValues[4] > high && sensorValues[5] > high)
  {
    ...
    do {
      ...
      position = readline();
    } while (sensorValues[0] > high && sensorValues[7] > high);
  }
  ...
  else if (sensorValues[0] > high && sensorValues[1] > high && sensorValues[2] > high && sensorValues[3] > high && sensorValues[4] > high)
  {
    ...
    do
    {
      bluetooth();
      readline();
    } while (sensorValues[0] > high);
    if (sensorValues[3] > high && sensorValues[4] > high)
      follow_line(position);
    else
      left_turn(d);
  }
  //right_turn && hexagonal
  else if (sensorValues[7] > high && sensorValues[6] > high && sensorValues[5] > high && sensorValues[4] > high && sensorValues[3] > high)
  {
    analogWrite(l_pwm, 0);
    analogWrite(r_pwm, 0);
    digitalWrite(l_motor_b, LOW);
