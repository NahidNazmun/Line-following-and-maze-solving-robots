...
/*for readline port c is used
   ...
   right most sensor is connected to pin 33 and the value is saved in senssor[0]
*/
...
void loop() {
  read_whiteline();
  if (sensor_value[0] && sensor_value[6] && sensor_value[2] && sensor_value[3] && sensor_value[4] && sensor_value[7])
  {
    digitalWrite(l_motor_f, HIGH);
  ...
  //  if (sensor_value[0] && sensor_value[1] && sensor_value[2] && sensor_value[3])
  //  {
  //    digitalWrite(l_motor_f, HIGH);
  //    digitalWrite(l_motor_b, LOW);
  //    digitalWrite(r_motor_f, HIGH);
  ...
  //  }
  //
  //  if (sensor_value[7] && sensor_value[6] && sensor_value[5] && sensor_value[4])
  //  {
  //    digitalWrite(l_motor_f, HIGH);
  //    digitalWrite(l_motor_b, LOW);
  //    digitalWrite(r_motor_f, HIGH);
...
void turn_left()
{
  ...
  do {
    read_whiteline();
  } while (sensor_value[4] == LOW && sensor_value[5] == LOW);
  digitalWrite(l_motor_f, HIGH);
  digitalWrite(l_motor_b, LOW);
  last_speed_var = 0;
}
...
void turn_right()
{
  ...
  do {
    read_whiteline();
  } while (sensor_value[2] == LOW && sensor_value[3] == LOW);
  digitalWrite(r_motor_f, HIGH);
  digitalWrite(r_motor_b, LOW);
  last_speed_var = 0;
}
